...
hdlr = logging.FileHandler(os.path.join(cur_path, 'antifraud_bfs.log'))
...
output_stream = os.path.relpath('..\\paymo_output\\output1_bfs.txt', cur_path)
...
with open(output_stream, 'w') as fout:
    with open(input_stream, 'r') as fin:
        for line in fin:
            ...
            i += 1
            fout.write(verify_feat1(connections, user_1, user_2) + "\n")
